...
void main() {
    ...
    int specExp = 100;
	vec4 ambient = Ia * ka;
	vec4 diffuse = Id * kd * max(0, clamp(dot(vertexNormal, ToLightVector), 0, 1));
    vec4 specular = Is * ks * pow(max(0, clamp(dot(vertexNormal, normalize(ToLightVector + ToCameraVector)), 0, 1)), specExp);
